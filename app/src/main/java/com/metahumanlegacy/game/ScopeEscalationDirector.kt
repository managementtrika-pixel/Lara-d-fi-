...
internal object ScopeEscalationDirector {
    ...
    fun enrich(c: Campaign, base: EventNode): EventNode {
        ...
        val ordinaryLimit = (6 - protected.size).coerceAtLeast(0)
        val retained = ordinary.take(ordinaryLimit) + protected
        return base.copy(
    ...
    private fun responseFor(scope: Scope, event: EventNode): Choice = when (scope) {
        Scope.STREET -> Choice(
            ...
            moral = 1, opinion = 1, impact = 1,
            risk = event.stakes.coerceAtLeast(1), approach = "CARE", stakes = event.stakes,
            sourceCategory = event.category, relationDelta = 1, identityDelta = 1,
            flag = "${FLAG_PREFIX}street"
        )
        Scope.DISTRICT -> Choice(
            label = "Mobiliser les relais du quartier au lieu d'agir seul",
            moral = 1, opinion = 1, prestige = 1, impact = 2,
            risk = event.stakes.coerceAtLeast(1), approach = "CARE", stakes = event.stakes,
            sourceCategory = event.category, relationDelta = 2,
            flag = "${FLAG_PREFIX}district"
        )
        Scope.CITY -> Choice(
            label = "Coordonner secours, accès et infrastructures à l'échelle de la ville",
            prestige = 2, opinion = 1, impact = 3,
            risk = (event.stakes - 1).coerceAtLeast(1), approach = "ORDER", stakes = event.stakes,
            sourceCategory = event.category, flag = "${FLAG_PREFIX}city"
        )
        Scope.REGION -> Choice(
            label = "Prioriser les zones menacées et déléguer ce que tu ne peux pas couvrir",
            moral = 1, prestige = 2, impact = 4,
            risk = event.stakes, approach = "ORDER", stakes = event.stakes,
            sourceCategory = event.category, relationDelta = 1,
        ...
        Scope.COUNTRY -> Choice(
            ...
            opinion = 2, prestige = 2, impact = 5,
            risk = event.stakes, approach = "TRUTH", stakes = event.stakes,
            sourceCategory = event.category, identityDelta = 1,
            flag = "${FLAG_PREFIX}country"
        )
        Scope.WORLD -> Choice(
            label = "Synchroniser plusieurs réponses internationales et accepter de ne pas être partout",
            moral = 1, opinion = 2, prestige = 3, impact = 6,
            risk = event.stakes.coerceAtLeast(2), approach = "CARE", stakes = event.stakes,
            sourceCategory = event.category, relationDelta = 1, identityDelta = 1,
